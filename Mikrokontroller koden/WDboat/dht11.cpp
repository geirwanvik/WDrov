...
void _DHT::Init(uint8_t pin, DHT_MODEL_t model)
{
	_DHT::pin = pin;
	_DHT::model = model;
	_DHT::ResetTimer(); // Make sure we do read the sensor in the next readSensor()
...
void _DHT::ResetTimer()
{
	_DHT::lastReadTime = millis() - 3000;
}

const char* _DHT::getStatusString()
{
	switch (error) {
	case _DHT::ERROR_TIMEOUT:
		return "TIMEOUT";

	case _DHT::ERROR_CHECKSUM:
		return "CHECKSUM";
